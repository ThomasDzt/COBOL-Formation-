      ...
      *Création d'une variable de nom saisi par l'utilisateur
       01  WS-NOM-SAISI            PIC X(10).

      *Création d'une variable qui stocke l'âge de la personne concernée
       01  WS-AGE                  PIC 9(2).
      ...
      *Saisie du nom par l'utilisateur 
       ...
       DISPLAY "Saisissez un nom :".
       ACCEPT WS-NOM-SAISI.

       PERFORM VARYING WS-INDEX-TABLEAU FROM 1 BY 1 
               UNTIL WS-INDEX-TABLEAU > WS-MAX-TABLEAU 

           IF WS-NOM-SAISI = WS-NOM(WS-INDEX-TABLEAU) 
               COMPUTE WS-AGE = 2025 - WS-ANNEE(WS-INDEX-TABLEAU)
